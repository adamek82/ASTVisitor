...
double EvaluatingVisitorRecursive::evaluate(Node* node) {
...
// Handle numbers
void EvaluatingVisitorRecursive::visit(NodeNumber* node) {
    currentValue = node->value;
}

// Handle addition
void EvaluatingVisitorRecursive::visit(NodePlus* node) {
...
// Handle subtraction
void EvaluatingVisitorRecursive::visit(NodeMinus* node) {
    double leftVal, rightVal;
    leftVal = evaluate(node->left.get());
    rightVal = evaluate(node->right.get());
    currentValue = leftVal - rightVal;
...
void EvaluatingVisitorRecursive::visit(NodeMultiply* node) {
    double leftVal, rightVal;
    leftVal = evaluate(node->left.get());
    rightVal = evaluate(node->right.get());
    currentValue = leftVal * rightVal;
...
// Handle division
void EvaluatingVisitorRecursive::visit(NodeDivide* node) {
    double leftVal, rightVal;
    leftVal = evaluate(node->left.get());
    rightVal = evaluate(node->right.get());
...
// Handle exponent
void EvaluatingVisitorRecursive::visit(NodePower* node) {
...
void EvaluatingVisitorRecursive::visit(NodeAssign* node) {
...
void EvaluatingVisitorRecursive::visit(NodeVarRef* node) {
